...
const required = [
  ...
  "scripts/predeploy.mjs","scripts/configure-production.mjs","tests/integration.test.mjs","tests/client.test.mjs","RELEASE-NOTES-1.4.1.md","V1.4.1-COMPLETION-REPORT.md","RELEASE-NOTES-1.4.2.md","V1.4.2-ACTIONS-REPORT.md","RELEASE-NOTES-1.4.3.md","V1.4.3-AUTOMATIC-CODES-REPORT.md"
];
for (const file of required) if (!fs.existsSync(path.join(root, file))) throw new Error(`Missing ${file}`);

const pkg = JSON.parse(fs.readFileSync(path.join(root, "package.json"), "utf8"));
if (pkg.version !== "1.4.3") throw new Error("Unexpected package version");
...
if (!/"APP_VERSION"\s*:\s*"1\.4\.3"/.test(wrangler)) throw new Error("wrangler APP_VERSION does not match package version");
...
if (!/APP_VERSION\s*=\s*"1\.4\.3"/.test(config)) throw new Error("Runtime APP_VERSION does not match package version");
...
for (const route of ["/terms","/offerings","/enrollments","/enrollments/new","/offerings/:id/enroll","/enrollments/:id/status","/enrollments/:id/delete","/students/:id/status","/students/bulk","/terms/:id/status","/terms/:id/delete","/offerings/:id/status","/offerings/:id/delete","/sessions/:id/revoke","/users/:id/status","/users/:id/reset-requests/resolve"]) if (!index.includes(`"${route}"`)) throw new Error(`Required route missing: ${route}`);
if (!index.includes('router.add("POST",`/${key}/bulk`')) throw new Error("Dynamic academic bulk route missing");
if (!/Placement change conflicts/.test(index) || !/Cohort change would invalidate/.test(index)) throw new Error("Academic consistency guards missing");
...
console.log("VALIDATION PASS: V1.4.3 automatic codes + safe actions, schema v4, safe production deploy guard, connected academic network, consistency checks, reporting, session controls, enhanced import, version consistency, secret scan, zero runtime dependencies, syntax");
