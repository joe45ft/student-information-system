...
if (pkg.version !== "1.4.5") throw new Error("Unexpected package version");
...
if (!/"APP_VERSION"\s*:\s*"1\.4\.5"/.test(wrangler)) throw new Error("wrangler APP_VERSION does not match package version");
...
if (!/APP_VERSION\s*=\s*"1\.4\.5"/.test(config)) throw new Error("Runtime APP_VERSION does not match package version");
...
console.log("VALIDATION PASS: V1.4.5 friendly actionable errors + enrollment eligibility UX + automatic codes + safe actions, schema v4, production deploy guard, connected academic network, consistency checks, reporting, session controls, enhanced import, zero runtime dependencies, syntax");
