...
authRoutes.post("/logout",async c=>{if(requireUser(c))return c.redirect("/login");await formBody(c);await audit(c,"auth.logout","auth","user",c.get("user").id);await revokeCurrentSession(c);return c.redirect("/login?message="+encodeURIComponent("You have been logged out."))});
