...
const MOCK_ENV={AUTH_PEPPER:"test-pepper"};
...
const get=await app.request("http://test/setup",{},MOCK_ENV);assert.equal(get.status,200);const html=await get.text();assert.match(html,/method="post" action="\/setup"/);assert.match(html,new RegExp(`V${pkg.version.replaceAll(".","\\.")}`));assert.doesNotMatch(html,/fetch\("\/setup"/);
...
const post=await app.request("http://test/setup",{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded","cookie":csrfCookie},body:body.toString()},MOCK_ENV);assert.equal(post.status,303);assert.match(post.headers.get("location")||"",/^\/login\?message=/);assert.equal(state.user.role,"OWNER");assert.equal(state.user.full_name,"Test Owner");assert.equal(state.user.email,"owner@example.com");assert.equal(state.sessions,0);
console.log("TEST PASS: GET /setup -> native POST /setup -> Owner -> 303 /login");
